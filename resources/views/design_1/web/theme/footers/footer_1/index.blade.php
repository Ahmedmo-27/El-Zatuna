                        ...
                        <div class="col-6 col-lg-2 mt-32 mt-lg-0">
                            ...
                            @if(!empty($themeFooterContent['specific_links']) and is_array($themeFooterContent['specific_links']))
                                @foreach($themeFooterContent['specific_links'] as $specificLink1Data)
                                    @php
                                        $linkTitle = $specificLink1Data['title'] ?? '';
                                        $linkUrl = $specificLink1Data['url'] ?? '';
                                        $titleLower = mb_strtolower($linkTitle);
                                        $urlLower = mb_strtolower($linkUrl);

                                        $isDepartmentsLink = (str_contains($urlLower, 'department') || str_contains($titleLower, 'department'));
                                        $isAuthHiddenLink = (auth()->check() && (
                                            str_contains($urlLower, 'login') ||
                                            str_contains($urlLower, 'register') ||
                                            str_contains($urlLower, 'sign-up') ||
                                            str_contains($urlLower, 'sign_up') ||
                                            str_contains($titleLower, 'sign up') ||
                                            str_contains($titleLower, 'log in') ||
                                            str_contains($titleLower, 'login') ||
                                            str_contains($titleLower, 'register')
                                        ));
                                    @endphp

                                    @if(!empty($linkTitle) and !empty($linkUrl) and !$isDepartmentsLink and !$isAuthHiddenLink)
                                        <a href="{{ $linkUrl }}" target="_blank" class="btn-flip-effect btn-flip-effect__slow-effect btn-flip-effect__left-0 d-block font-16 text-white opacity-70 {{ $loop->first ? 'mt-16' : 'mt-12' }}" data-text="{{ $linkTitle }}">
                                            <span class="btn-flip-effect__text">{{ $linkTitle }}</span>
                                        </a>
                                    @endif
                                @endforeach
                            @endif
                        ...
                        <div class="col-6 col-lg-2 mt-32 mt-lg-0">
                            ...
                            @if(!empty($themeFooterContent['specific_links_2']) and is_array($themeFooterContent['specific_links_2']))
                                @foreach($themeFooterContent['specific_links_2'] as $specificLink2Data)
                                    @php
                                        $linkTitle = $specificLink2Data['title'] ?? '';
                                        $linkUrl = $specificLink2Data['url'] ?? '';
                                        $titleLower = mb_strtolower($linkTitle);
                                        $urlLower = mb_strtolower($linkUrl);
