        ...
        <div class="js-content-form session-form" data-action="/panel/sessions/{{ !empty($session) ? $session->id . '/update' : 'store' }}">
            ...
            <div class="form-group mt-20">
                ...
                <div class="js-session-api d-flex align-items-center mt-12">
                    @php
                        $availableSessionApis = getFeaturesSettings("available_session_apis");
                        if (is_string($availableSessionApis)) {
                            $decodedSessionApis = json_decode($availableSessionApis, true);
                            if (is_array($decodedSessionApis)) {
                                $availableSessionApis = $decodedSessionApis;
                            } else {
                                $availableSessionApis = array_filter(array_map('trim', explode(',', $availableSessionApis)));
                            }
                        }
                        if (!is_array($availableSessionApis) || empty($availableSessionApis)) {
                            $availableSessionApis = ['local'];
                        }
                    @endphp
                    @foreach($availableSessionApis as $sessionApi)
