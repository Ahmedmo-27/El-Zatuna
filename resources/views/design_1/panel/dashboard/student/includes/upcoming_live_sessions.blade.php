<div class="bg-white p-16 rounded-24 mt-24">
    ...
    @if(!empty($upcomingLiveSessions['totalSessions']))
        ...
        @foreach($upcomingLiveSessions['sessions'] as $upcomingLiveSession)
            @php
                $sessionWebinar = $upcomingLiveSession->webinar;
            @endphp
            <div class="bg-gray-100 rounded-16 p-12 mt-16">
                <div class="d-flex align-items-center">
                    <div class="size-48 rounded-12">
                        <img src="{{ !empty($sessionWebinar) ? $sessionWebinar->getIcon() : '/assets/default/img/activity/48.svg' }}" alt="" class="rounded-12 img-cover">
                    </div>
                    <div class="ml-8">
                        <h6 class="font-14 text-dark">{{ truncate($upcomingLiveSession->title, 22) }}</h6>
                        <p class="font-12 text-gray-500 mt-4">{{ truncate($sessionWebinar->title ?? trans('public.course'), 25) }}</p>
                    </div>
                </div>
                ...
                <div class="bg-white py-16 rounded-12 mt-16">
                    <div class="d-flex align-items-center px-16">
                        ...
                        <div class="ml-8">
                            ...
                            <span class="d-block font-12 text-gray-500 mt-2">{{ $upcomingLiveSession->creator->role->caption ?? $upcomingLiveSession->creator->role_name }}</span>
                        </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center justify-content-between mt-16 px-16 pt-16 border-top-gray-100">
                        ...
                        <a href="{{ !empty($sessionWebinar) ? $sessionWebinar->getLearningPageUrl() . '?type=session&item=' . $upcomingLiveSession->id : '#!' }}" target="_blank" class="d-flex-center size-40 rounded-circle bg-primary">
