...
(function ($) {
  ...
  window.makeVideoPlayerHtml = function (path, storage, height, tagId) {
    ...
    var mimeType = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;
    var controls = ['play-large', 'rewind', 'play', 'fast-forward', 'progress', 'current-time', 'duration', 'mute', 'volume', 'settings', 'fullscreen'];
    var resolvedHeight = height === null || height === undefined || height === '' ? '100%' : height;
    var isDirectVideoSource = function isDirectVideoSource(sourcePath) {
      var sourceMimeType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      var normalizedMimeType = (sourceMimeType || '').toLowerCase();
    ...
    if (storage === 'youtube') {
      ...
      if (isDirectVideoSource(path, mimeType)) {
        var sourceType = mimeType || 'video/mp4';
        html = "<video id=\"".concat(tagId, "\" class=\"plyr-io-video\" controls preload=\"auto\" width=\"100%\" height=\"").concat(resolvedHeight, "\" data-poster=\"").concat(thumbnail !== null && thumbnail !== void 0 ? thumbnail : '', "\">\n                    <source src=\"").concat(path, "\" type=\"").concat(sourceType, "\"/>\n                </video>");
      } else {
        html = '<iframe src="' + path + '" class="img-cover bg-gray-200" frameborder="0" allowfullscreen="true" ></iframe>';
        usePlyr = false;
      }
    } else {
      var _sourceType = mimeType || 'video/mp4';
      html = "<video id=\"".concat(tagId, "\" class=\"plyr-io-video\" controls preload=\"auto\" width=\"100%\" height=\"").concat(resolvedHeight, "\" data-poster=\"").concat(thumbnail !== null && thumbnail !== void 0 ? thumbnail : '', "\">\n                <source src=\"").concat(path, "\" type=\"").concat(_sourceType, "\"/>\n            </video>");
    }
    return {
      html: html,
      options: options,
  ...
  window.handleVideoByFileId = function (fileId, $contentEl, callback) {
    ...
    $.post('/course/getFilePath', {
      ...
      if (result && result.code === 200) {
        ...
        var _makeVideoPlayerHtml = makeVideoPlayerHtml(result.path, storage, '100%', videoTagId, undefined, result.mime_type),
          html = _makeVideoPlayerHtml.html,
          options = _makeVideoPlayerHtml.options,
          usePlyr = _makeVideoPlayerHtml.usePlyr;
        if ($contentEl) {
          $contentEl.html(html);
        }
        if (usePlyr) {
