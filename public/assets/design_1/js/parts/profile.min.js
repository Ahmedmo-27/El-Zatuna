    ...
    $.post(path, {}, function (result) {
      if (result && result.code === 200) {
        ...
        if (typeof result.followers_count_short !== 'undefined') {
          $('.js-user-followers-count').text(result.followers_count_short).attr('data-raw-count', result.followers_count);
        }
        if (typeof result.following_count_short !== 'undefined') {
          $('.js-user-following-count').text(result.following_count_short).attr('data-raw-count', result.following_count);
        }
      }
    }).fail(function (xhr) {
      var title = 'Request failed';
      var message = xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Please try again.';
