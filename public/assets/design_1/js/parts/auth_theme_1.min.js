...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    var $uniSelect = $('.js-university-select');
    var $facultySelect = $('.js-faculty-select');
    if ($uniSelect.length && $facultySelect.length) {
      var uniVal = $uniSelect.val();
      if (uniVal) {
        var fromData = $facultySelect.data('selected-faculty');
        fillFacultySelectForUniversity(uniVal, {
          preserveFacultyId: fromData !== undefined && fromData !== null && String(fromData) !== '' ? fromData : ''
        });
      }
    }
  });
  ...
  * **************/
  ...
  function getFacultiesListForUniversity(universityId) {
    ...
    var map = window.facultiesByUniversity;
    if (map[universityId]) {
  ...
  function fillFacultySelectForUniversity(universityId, options) {
    options = options || {};
    var preserveRaw = options.preserveFacultyId;
    var preserveFacultyId = preserveRaw !== undefined && preserveRaw !== null && String(preserveRaw) !== '' ? String(preserveRaw) : '';
    var $facultySelect = $('.js-faculty-select');
    if (!$facultySelect.length) {
      return;
    }
    var defaultOptionText = $facultySelect.find('option').first().text() || 'Select';
    var selectPlaceholder = !preserveFacultyId ? ' selected' : '';
    $facultySelect.html("<option value=\"\" disabled".concat(selectPlaceholder, ">").concat(defaultOptionText, "</option>"));
    if (!universityId) {
      return;
    }
    var applyFaculties = function applyFaculties(faculties) {
      faculties.forEach(function (faculty) {
        $facultySelect.append("<option value=\"".concat(faculty.id, "\">").concat(faculty.name, "</option>"));
      });
      if (preserveFacultyId) {
        $facultySelect.val(preserveFacultyId);
      }
    };
    var preloaded = getFacultiesListForUniversity(universityId);
    if (preloaded) {
      applyFaculties(preloaded);
      return;
    }
    $.get("/universities/".concat(universityId, "/faculties"), function (result) {
  ...
  $('body').on('change', '.js-university-select', function () {
    fillFacultySelectForUniversity($(this).val(), {
      preserveFacultyId: ''
    });
  });
})(jQuery);
