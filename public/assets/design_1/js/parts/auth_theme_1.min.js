...
(function ($) {
  ...
  * **************/
  ...
  $('body').on('change', '.js-university-select', function () {
    ...
    $facultySelect.html("<option value=\"\" disabled selected>".concat(defaultOptionText, "</option>"));
    if (!universityId) {
      return;
    }
    ...
    if (window.facultiesByUniversity && window.facultiesByUniversity[universityId]) {
      var faculties = window.facultiesByUniversity[universityId];
      ...
      $.get("/universities/".concat(universityId, "/faculties"), function (result) {
        if (result && result.code === 200) {
          var _faculties = result.faculties || [];
          _faculties.forEach(function (faculty) {
            $facultySelect.append("<option value=\"".concat(faculty.id, "\">").concat(faculty.name, "</option>"));
          });
        }
      });
