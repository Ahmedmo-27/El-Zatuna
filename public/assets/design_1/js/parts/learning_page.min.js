...
(function ($) {
    ...
    if ($players.length) {
      ...
      try {
        var _loop = function _loop() {
          ...
          var player = new Plyr(plyr, getLearningPlyrOptions());

          // Add video completion tracking
          var $playerElement = $(plyr);
          var videoId = $playerElement.attr('id');
          if (videoId && videoId.includes('fileVideo')) {
            var fileId = videoId.replace('fileVideo', '');
   ...
   * */
  var autoCompletedItems = new Set();
  function autoMarkItemComplete(itemType, itemId) {
    var key = "".concat(itemType, "_").concat(itemId);

    // Check if already auto-completed in this session
    if (autoCompletedItems.has(key)) {
      return;
    }
    var path = "/course/learning/".concat(courseSlug, "/autoMarkComplete");
    var data = {
      item: itemType,
      item_id: itemId
    };
    $.post(path, data, function (result) {
      if (result.code === 200 && !result.already_completed) {
        ...
        var $checkbox = $(".js-passed-item-toggle[data-item-name=\"".concat(itemType, "\"][value=\"").concat(itemId, "\"]"));
  ...
  function handleDocumentScrollTracking() {
    var $scrollContainer = $('.learning-page__main-content .simplebar-content-wrapper');
    var $textLessonContent = $('.js-text-lesson-content');
    if ($textLessonContent.length && $scrollContainer.length) {
      var textLessonId = $textLessonContent.attr('data-text-lesson-id');
      if (textLessonId) {
        ...
        $scrollContainer.on('scroll.textLesson', function () {
          ...
          var clientHeight = $scrollContainer.height();
