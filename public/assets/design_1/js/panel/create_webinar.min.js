...
(function ($) {
  ...
  function showMp4OnlyMessage() {
    var msg = 'Please convert the file to mp4 before uploading';
  ...
  function initPanelCategoryInput() {
    var $wrapper = $('.js-panel-category-wrapper');
    if (!$wrapper.length) return;
    var $input = $wrapper.find('.js-panel-category-input');
    var $dropdown = $wrapper.find('.js-panel-category-dropdown');
    var $results = $wrapper.find('.js-panel-category-results');
    var $addNew = $wrapper.find('.js-panel-category-add-new');
    var $addNewTerm = $wrapper.find('.js-panel-category-add-new-term');
    var $tagContainer = $wrapper.find('.js-panel-category-tag');
    var $hidden = $wrapper.find('.js-panel-category-hidden');
    var $loading = $wrapper.find('.js-panel-category-loading');
    var $error = $wrapper.find('.js-panel-category-error');
    var selected = null;
    var searchTimeout = null;
    var hideDropdownTimeout = null;
    function setLoading(isLoading) {
      if (isLoading) $loading.removeClass('d-none');else $loading.addClass('d-none');
    }
    ...
    function renderTag() {
      $tagContainer.empty();
      if (selected) {
        var textEsc = $('<div>').text(selected.text).html();
        var $tag = $('<span class="badge bg-[#F5F9E8] text-[#072923] px-3 py-1 rounded-8 d-inline-flex align-items-center gap-1">' + '<span>' + textEsc + '</span>' + '<button type="button" class="js-panel-category-remove btn btn-link p-0 text-[#072923]/60 hover:text-danger" style="font-size: 14px; line-height: 1;" data-id="' + selected.id + '">&times;</button></span>');
    ...
    function doSearch(q) {
      ...
      $.get('/become-instructor/search-subjects', {
        q: q || ''
      }).done(function (data) {
        var results = data.results || [];
        $results.empty();
        if (!results.length) {
          if (q) $results.append($('<div class="p-2 text-[#072923]/50">No matching categories. Use "Add new category" below to add "' + $('<div>').text(q).html() + '".</div>'));else $results.append($('<div class="p-2 text-[#072923]/50">No categories found.</div>'));
        } else {
          results.forEach(function (item) {
            var textEsc = $('<div>').text(item.text).html();
        ...
        showError('Could not load categories. Please try again.');
      }).always(function () {
        if (q && q.length) setLoading(false);
      });
    }
    ...
    $addNew.on('click', function (e) {
      ...
      $.post('/become-instructor/create-subject', {
        ...
        showError('Could not create category. Please try again.');
      }).always(function () {
        setLoading(false);
      });
    });
  ...
  $('body').on('click', '.js-add-content-dropdown-toggle', function (e) {
    e.stopPropagation();
    var $this = $(this);
    var $dropdown = $this.closest('.actions-dropdown').find('.actions-dropdown__dropdown-menu');
  ...
  $('body').on('click', '.js-add-course-content-btn, .add-new-interactive-file-btn', function (e) {
    ...
    if ($body) {
      ...
      var $contentList = $contentTagId.find('ul.draggable-content-lists');
      ...
      var $newItem = $contentList.find('li').first();
      ...
      $newItem.find('[data-toggle="collapse"]').each(function () {
        ...
        var parentValue = '#chapterContentAccordion' + chapterId;
      ...
      if (type === "file" || type === "new_interactive_file") {
        ...
        setTimeout(function () {
          var $newForm = $contentList.find('.js-content-form').first();
          var $dragDropZone = $newForm.find('.js-file-drag-drop-zone');
          if ($dragDropZone.length) {
            ...
            if (typeof window.FileDragDrop !== 'undefined') {
              ...
              $dragDropZone.each(function () {
                var $zone = $(this);
                var fileInputId = $zone.data('file-input-id');
                ...
                if ($fileInput.length) {
                  ...
                  $zone.on('drop', function (e) {
                    var _e$originalEvent$data;
                    e.preventDefault();
                    e.stopPropagation();
                    $zone.removeClass('drag-over');
                    $zone.find('.js-drag-drop-overlay').addClass('d-none');
                    var files = (_e$originalEvent$data = e.originalEvent.dataTransfer) === null || _e$originalEvent$data === void 0 ? void 0 : _e$originalEvent$data.files;
          ...
          var $storageSelect = $newForm.find('.js-file-storage');
          if ($storageSelect.length) {
            // Get the selected storage value (default to 'r2' if not set)
            var storageValue = $storageSelect.val();
            if (!storageValue || storageValue === '') {
              storageValue = 'r2';
              $storageSelect.val('r2');
            }
            var $fileTypeSelect = $newForm.find('.js-ajax-file_type');
            var fileType = $fileTypeSelect.length ? $fileTypeSelect.val() : null;

            // Find the form container (could be .file-form or .js-content-form)
            var $formContainer = $newForm.closest('.file-form, .js-content-form');

            // Ensure upload input is visible for new files (it should be by default from Blade)
            var $uploadInput = $newForm.find('.js-file-upload-input');
      ...
      var $newlyAdded = $contentList.find('li').first();
  ...
  function syncAndValidateTextLessonContent($form) {
    ...
    var $editor = $form.find('textarea.js-content-summernote, textarea[class*="js-content-summernote-"]').first();
    var $hiddenContent = $form.find('.js-hidden-content-summernote, textarea[class*="js-hidden-content-summernote-"], textarea[name*="[content]"]').first();
    ...
    var hasMedia = /<(img|iframe|video|audio|object|embed)\b/i.test(html);
    var isEmpty = !hasMedia && extractSummernotePlainText(html).length === 0;
    var $contentFormGroup = $hiddenContent.closest('.form-group');
    var $feedback = $contentFormGroup.find('.invalid-feedback').first();
    if (isEmpty) {
      $editor.addClass('is-invalid');
      $hiddenContent.addClass('is-invalid');
      if ($feedback.length) {
        var message = typeof contentRequiredLang !== 'undefined' && contentRequiredLang ? contentRequiredLang : 'The content field is required.';
  ...
  $('body').on('click', '.js-save-course-content', function (e) {
    ...
    if ($form.hasClass('file-form')) {
      var storage = $form.find('.js-file-storage').val() || $form.find('select[name*="[storage]"]').val() || 'upload';
      if (storage === 'r2' || storage === 'upload') {
        var action = $form.attr('data-action') || '';
        var isPanelFileStore = action.indexOf('/panel/files/store') !== -1;
        var $fileInput = $form.find('.js-ajax-upload-file-input');
        var hasFile = $fileInput.length && $fileInput[0].files && $fileInput[0].files.length > 0;
  ...
  function showUploadErrorForForm($form, title, message, options) {
    var opts = options || {};
    var $errorTarget = $form.find('.js-file-upload-input .invalid-feedback').first();
  ...
  function handleDirectR2UploadAndSubmit($form, $button, storage) {
    try {
      ...
      var maxSizeBytes = 2 * 1024 * 1024 * 1024; // 2GB
      if (file.size > maxSizeBytes) {
        showUploadErrorForForm($form, 'File too large', 'Maximum supported size is 2GB.');
        return;
      }
      var webinarId = $form.find('input[name="ajax[new][webinar_id]"]').val();
      var chapterId = $form.find('input[name="ajax[new][chapter_id]"]').val() || null;
      var token = $('meta[name="csrf-token"]').attr('content') || $('input[name="_token"]').val();
      if (!webinarId) {
        showUploadErrorForForm($form, 'Missing course', 'Course ID is missing. Please refresh the page and try again.');
        return;
      }
      var $progressContainer = $form.find('.progress').first();
      var $progressBar = $progressContainer.length ? $progressContainer.find('.progress-bar') : null;
      ...
      $.ajax({
        ...
        success: function success(res) {
          ...
          if ($progressContainer.length && $progressBar && $progressBar.length) {
            ...
            $progressBar.css('width', '0%').attr('aria-valuenow', 0).removeClass('bg-danger').addClass('bg-primary');
          }
          ...
          xhr.onerror = function () {
            $button.removeClass('loadingbar').prop('disabled', false);
            if ($progressContainer.length && $progressBar && $progressBar.length) {
              $progressBar.addClass('bg-danger').removeClass('bg-primary').css('width', '100%').attr('aria-valuenow', 100);
            }
          ...
          xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 300) {
              ...
              try {
                var emptyDataTransfer = new DataTransfer();
        ...
        error: function error(err) {
          $button.removeClass('loadingbar').prop('disabled', false);
          var msg = 'Could not prepare upload. Please try again.';
          if (err && err.responseJSON) {
            if (err.responseJSON.msg) {
              msg = err.responseJSON.msg;
            } else if (err.responseJSON.errors) {
              var firstKey = Object.keys(err.responseJSON.errors)[0];
  ...
  $('body').on('change', '.js-file-storage', function (e) {
    ...
    var value = $(this).val();
    // Find the form container (could be .file-form or .js-content-form)
    var formGroup = $(this).closest('.file-form, .js-content-form');
