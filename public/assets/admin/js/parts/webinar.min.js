...
(function ($) {
  ...
  function initAdminCategoryInput() {
    var $wrapper = $('.js-admin-category-input');
    if (!$wrapper.length) return;
    var $search = $wrapper.find('.js-admin-category-search');
    var $dropdown = $wrapper.find('.js-admin-category-dropdown');
    var $results = $wrapper.find('.js-admin-category-results');
    var $addNew = $wrapper.find('.js-admin-category-add-new');
    var $addNewTerm = $wrapper.find('.js-admin-category-add-new-term');
    var $tagContainer = $wrapper.find('.js-admin-category-tag');
    var $hidden = $wrapper.find('.js-admin-category-hidden');
    var searchUrl = $wrapper.data('search-url');
    var quickStoreUrl = $wrapper.data('quick-store-url');
    var selected = null;
    ...
    function renderTag() {
      ...
      if (selected) {
        ...
        var $tag = $('<span class="badge badge-primary d-inline-flex align-items-center px-3 py-1">' + '<span>' + textEsc + '</span>' + ' <button type="button" class="js-admin-category-remove btn btn-link p-0 ml-1 text-white" style="font-size: 14px; line-height: 1;" data-id="' + selected.id + '">&times;</button>' + '</span>');
      ...
      $.get(searchUrl, {
        ...
        if (list.length === 0) {
          ...
          list.forEach(function (item) {
            ...
            var id = item.id;
            var textEsc = $('<div>').text(title).html();
            var $row = $('<div class="js-admin-category-result p-2 rounded cursor-pointer hover:bg-light" data-id="' + id + '" data-title="' + $('<div>').text(title).html() + '">' + textEsc + '</div>');
            $results.append($row);
          });
        }
      });
    ...
    $addNew.on('click', function () {
      var term = $.trim($search.val()) || $.trim($addNewTerm.text());
