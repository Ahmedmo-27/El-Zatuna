            ...
            foreach ($items as $item) {
                try {
                    ...
                    if ($itemType === 'course' || $itemType === 'webinar') {
                        // Add course/webinar to cart - bypass global scope
                        $webinar = Webinar::withoutGlobalScope('universityFaculty')
                            ->where('id', $itemId)
                            ->where('private', false)
                            ->where('status', 'active')
                            ->first();
                        ...
                        if (!$webinar) {
                            $errors[] = ['item_id' => $itemId, 'error' => 'Course not found or not available'];
                            continue;
                        }

                        $checkCourseForSale = $webinar->checkWebinarForSale($user, true);
                        if ($checkCourseForSale != 'ok') {
                            $errors[] = ['item_id' => $itemId, 'error' => $checkCourseForSale];
                            continue;
                        }
                        ...
                        // Add bundle to cart
                        $bundle = \App\Models\Bundle::where('id', $itemId)
                            ->where('status', 'active')
                            ->first();

                        if (!$bundle) {
                            $errors[] = ['item_id' => $itemId, 'error' => 'Bundle not found or not available'];
                            continue;
                        }

                        $checkBundleForSale = $bundle->checkWebinarForSale($user);
                        if ($checkBundleForSale != 'ok') {
                            $errors[] = ['item_id' => $itemId, 'error' => $checkBundleForSale];
                            continue;
                        }
                        ...
                        if (!$product) {
                            $errors[] = ['item_id' => $itemId, 'error' => 'Product not found or not available'];
                            continue;
                        }

                        $checkProductForSale = $product->checkProductForSale($user);
                        if ($checkProductForSale != 'ok') {
                            $errors[] = ['item_id' => $itemId, 'error' => $checkProductForSale];
                            continue;
                        }
                    ...
                    $errors[] = ['item_id' => $item['item_id'], 'error' => $e->getMessage()];
                }
            }

            return apiResponse2(1, 'bulk_added', trans('api.public.stored'), [
        ...
        try {
            ...
            foreach ($cartItemIds as $cartId) {
                try {
                    ...
                    if (!$cart) {
                        $errors[] = ['cart_id' => $cartId, 'error' => 'Cart item not found'];
                        continue;
                    }

                    // Delete associated reserve meeting if exists
                    if (!empty($cart->reserve_meeting_id)) {
                        $reserve = ReserveMeeting::where('id', $cart->reserve_meeting_id)
                    ...
                    $errors[] = ['cart_id' => $cartId, 'error' => $e->getMessage()];
                }
            }

            return apiResponse2(1, 'bulk_removed', trans('api.public.deleted'), [
                'deleted_count' => $deletedCount,
                'total_requested' => count($cartItemIds),
                'errors' => $errors,
            ]);
