...
class CommentsController extends Controller
{
    ...
    public function myClassComments(Request $request)
    {
        ...
        // Mark unviewed comments as viewed
        $unviewedIds = (clone $query)->whereNull('viewed_at')->pluck('id');
        if ($unviewedIds->isNotEmpty()) {
            Comment::whereIn('id', $unviewedIds)->update(['viewed_at' => time()]);
        }
        ...
        $paginatedData = apiPagination(
            ...
            url('/api/' . config('app.api_version', 'development') . '/panel/comments/class')
        );
    ...
    public function myComments(Request $request)
    {
        ...
        $webinar_comments = apiPagination(
            $webinar_query->whereNotNull('webinar_id')->handleFilters(),
            $request,
            function ($comment) {
                return $comment->details;
            },
            url('/api/' . config('app.api_version', 'development') . '/panel/comments/my')
        );

        $blog_query = clone $query;
        $blog_comments = apiPagination(
            $blog_query->whereNotNull('blog_id')->handleFilters(),
            $request,
            function ($comment) {
                return $comment->details;
            },
