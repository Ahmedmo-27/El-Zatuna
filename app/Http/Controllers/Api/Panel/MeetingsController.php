...
class MeetingsController extends Controller
{
    ...
    public function reservation(Request $request)
    {
        ...
        $filteredQuery = $this->filters(deepClone($reserveMeetingsQuery), $request);
        $filteredQuery = $filteredQuery->with([
            'meetingTime',
        ...
        $paginatedData = apiPagination(
            $filteredQuery,
            $request,
            null, // No transformer needed, return as-is
            url('/api/' . config('app.api_version', 'development') . '/panel/meetings')
        );

        $data = [
            'instructors' => $instructors,
            'reserveMeetings' => $paginatedData['items'],
