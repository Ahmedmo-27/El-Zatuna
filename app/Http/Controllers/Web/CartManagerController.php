...
class CartManagerController extends Controller
{
            ...
            if (!empty($cookieCarts)) {
                ...
                if (!empty($cookieCarts) and count($cookieCarts)) {
                    ...
                    foreach ($cookieCarts as $id => $cookieCart) {
                        ...
                        } elseif (!empty($cookieCart['item_name']) and $cookieCart['item_name'] == 'file_id') {
                            $file = File::where('id', $cookieCart['item_id'])
                                ->where('status', File::$Active)
                                ->with('webinar')
                                ->first();

                            if (!empty($file) and !empty($file->webinar)) {
                                $item = new Cart();
                                $item->uid = $id;
    ...
    public function storeUserFileCart($user, $data)
    {
        $fileId = $data['item_id'];
        ...
        if (!empty($file) and !empty($file->webinar) and !empty($user)) {
            if ($file->accessibility != 'paid' || empty($file->price) || $file->price <= 0) {
            ...
            if ($file->webinar->checkUserHasBought($user) || $file->checkUserHasBought($user)) {
                return back()->with(['toast' => [
