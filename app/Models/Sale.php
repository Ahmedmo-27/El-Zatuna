...
class Sale extends Model
{
    ...
    public static function createSales($orderItem, $payment_method)
    {
        ...
        if (!empty($orderItem->webinar_id) && $orderType === Order::$webinar) {
            (new \App\Services\TutorRevenueService())->recordCourseSale($orderItem, $sale);
        }

        if (!empty($orderItem->file_id) && $orderType === Order::$file) {
