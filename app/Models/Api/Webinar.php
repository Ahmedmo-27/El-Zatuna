...
class Webinar extends Model
{
    ...
    public function scopeHandleFilters($query)
    {
        ...
        // Allow filtering by university and faculty from API request
        // Include webinars that have NULL university/faculty (global) as well
        $universityId = $request->get('university_id', null);
        $facultyId = $request->get('faculty_id', null);

        if (!empty($universityId) && is_numeric($universityId)) {
            $query->where(function ($q) use ($universityId) {
                $q->whereNull('webinars.university_id')
                    ->orWhere('webinars.university_id', $universityId);
            });
        }

        if (!empty($facultyId) && is_numeric($facultyId)) {
            $query->where(function ($q) use ($facultyId) {
                $q->whereNull('webinars.faculty_id')
                    ->orWhere('webinars.faculty_id', $facultyId);
            });
        }
