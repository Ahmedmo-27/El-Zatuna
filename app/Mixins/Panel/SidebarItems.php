...
class SidebarItems
{
    ...
    static public function getFinancialSectionItems($user)
    {
        ...
        if ($user->can('panel_financial')) {
            ...
            if (($user->isOrganization() || $user->isTeacher()) and $user->can('panel_financial_sales_reports')) {
                $items['financial']['items'][] = ['text' => trans('update.tutor_earnings'), 'url' => '/panel/financial/tutor-earnings'];
            }

            if (($user->isOrganization() || $user->isTeacher()) and $user->can('panel_financial_payout')) {
                $items['financial']['items'][] = ['text' => trans('update.tutor_payouts'), 'url' => '/panel/financial/tutor-payouts'];
            }
