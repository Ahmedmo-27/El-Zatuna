...
class CartItemInfo
{
    ...
    private function getFileInfo($cart, $file)
    {
        $info = [];
        ...
        $info['profileUrl'] = !empty($webinar) ? $webinar->teacher->getProfileUrl() : null;
        $info['teacherName'] = !empty($webinar) ? $webinar->teacher->full_name : null;
        $info['rate'] = !empty($webinar) ? $webinar->getRate() : null;
        $info['rateCount'] = !empty($webinar) ? $webinar->reviews()->pluck('creator_id')->count() : null;
        $info['price'] = $file->price;
