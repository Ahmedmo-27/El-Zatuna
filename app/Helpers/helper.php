...
function convertPriceToUserCurrency($price, $userCurrencyItem = null)
{
    if (!is_numeric($price)) {
        ...
        $price = is_numeric($normalized) ? (float)$normalized : 0;
    }
...
function getDefaultAvatarPath($user = null)
{
    if ($user instanceof \App\User) {
        if ($user->isTeacher()) {
            return '/assets/admin/img/avatar/avatar-2.png';
        }

        return '/pfpfallback.png';
    }
