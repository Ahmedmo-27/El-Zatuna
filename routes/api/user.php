...
Route::group(['middleware' => 'api.auth'], function () {
    ...
    Route::get('/dashboard/summary', ['uses' => 'SummaryController@list']);
    ...
    // Consolidated subscription endpoint (replaces /apply and /general_apply)
    Route::post('/subscriptions', ['uses' => 'SubscribesController@apply']);
    ...
    Route::group(['prefix' => '/support'], function () {
        ...
        Route::patch('/{id}', ['uses' => 'SupportsController@update']);
    ...
    Route::group(['prefix' => 'favorites'], function () {
        ...
        Route::post('/toggle', ['uses' => 'FavoritesController@toggle']);
    ...
    Route::group(['prefix' => '/cart'], function () {
        ...
        Route::post('bulk-add', ['uses' => 'CartController@bulkAdd']);
        Route::delete('bulk-remove', ['uses' => 'CartController@bulkRemove']);
    ...
    Route::post('/account/deletion-request', 'DeleteAccountRequestController@store');
    ...
    Route::get('/purchases/offline', ['uses' => 'WebinarsController@offlinePurchases']);

    Route::post('/content-delete-request', 'ContentDeleteRequestController@store');
    
    // Active Session Management (Device Management)
    Route::group(['prefix' => '/active-sessions'], function () {
        Route::get('/', ['uses' => 'ActiveSessionsController@index']);
        Route::get('/stats', ['uses' => 'ActiveSessionsController@stats']);
        Route::delete('/', ['uses' => 'ActiveSessionsController@destroy']); // Pass session_token in body
        Route::post('/destroy-others', ['uses' => 'ActiveSessionsController@destroyOthers']); // Delete all except current
    });
    
    // Registered Devices Management
    Route::group(['prefix' => '/registered-devices'], function () {
        Route::get('/', ['uses' => 'RegisteredDevicesController@index']);
        Route::get('/stats', ['uses' => 'RegisteredDevicesController@stats']);
        Route::put('/{id}/trust', ['uses' => 'RegisteredDevicesController@updateTrust']);
        Route::delete('/{id}', ['uses' => 'RegisteredDevicesController@destroy']);
    });
    
    // Session Management (Legacy - kept for backward compatibility)
    Route::get('/sessions', ['uses' => 'UserSessionsController@index']);
    Route::delete('/sessions/{session_id}', ['uses' => 'UserSessionsController@destroy']);
    
    // Data Export (GDPR Compliance)
    Route::post('/account/export-data', ['uses' => 'DataExportController@requestExport']);
    Route::get('/account/export-data/{request_id}', ['uses' => 'DataExportController@getExport']);
    Route::get('/account/export-data/{request_id}/download', ['uses' => 'DataExportController@download']);
});
